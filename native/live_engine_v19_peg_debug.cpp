...
constexpr float VORTEX_SEVERITY_MULTIPLIER = 5.0f;
...
std::atomic<uint64_t> peg_repost_count{0};
...
// STRICT MAKER EXECUTION MODEL with adaptive pegging (Chasing Mid-Price)
// place_market_order is NEVER called in this file. Zero Taker Fee Rule.

inline double pegging_threshold(double spread) {
    double half_spread = 0.5 * spread;
    return std::max(half_spread, TICK_SIZE);
}
...
void execute_decision(Decision decision, float directed_pressure_proxy, const BookUpdate& update) {
    ...
    double threshold = pegging_threshold(spread);
    ...
    switch (decision) {
        ...
        case Decision::MARKET_TAKER: {
            ...
            Side chase_side = expect_up ? Side::BUY : Side::SELL;
            Side protect_side = expect_up ? Side::SELL : Side::BUY;

            double protect_price;
            if (matcher.get_open_order_price_for_side(protect_side, protect_price)) {
                matcher.cancel_orders_on_side(protect_side, tick);
            }

            double target_price = expect_up ? best_bid : best_ask;
            double chase_current_price;
            bool has_chase_order = matcher.get_open_order_price_for_side(chase_side, chase_current_price);

            if (!has_chase_order) {
                double qv = expect_up ? std::max(bid_vol, 0.001) : std::max(ask_vol, 0.001);
                matcher.place_limit_order(chase_side, target_price, ORDER_SIZE, qv, tick);
            } else {
                double drift = std::abs(chase_current_price - target_price);
                if (drift > threshold) {
                    peg_repost_count.fetch_add(1, std::memory_order_relaxed);
                    matcher.cancel_orders_on_side(chase_side, tick);
                    double qv = expect_up ? std::max(bid_vol, 0.001) : std::max(ask_vol, 0.001);
                    matcher.place_limit_order(chase_side, target_price, ORDER_SIZE, qv, tick);
                }
            }
        ...
        case Decision::LIQUIDITY_PROVISION: {
            double bid_current, ask_current;
            bool has_bid = matcher.get_open_order_price_for_side(Side::BUY, bid_current);
            bool has_ask = matcher.get_open_order_price_for_side(Side::SELL, ask_current);

            if (!has_bid || std::abs(bid_current - best_bid) > threshold) {
                if (has_bid) { peg_repost_count.fetch_add(1, std::memory_order_relaxed); matcher.cancel_orders_on_side(Side::BUY, tick); }
                matcher.place_limit_order(Side::BUY, best_bid, ORDER_SIZE, std::max(bid_vol, 0.001), tick);
            }
            if (!has_ask || std::abs(ask_current - best_ask) > threshold) {
                if (has_ask) { peg_repost_count.fetch_add(1, std::memory_order_relaxed); matcher.cancel_orders_on_side(Side::SELL, tick); }
                matcher.place_limit_order(Side::SELL, best_ask, ORDER_SIZE, std::max(ask_vol, 0.001), tick);
            }
            break;
        }
...
void hot_path_thread_fn() {
    ...
    while (!shutdown_flag.load(std::memory_order_relaxed)) {
        if (ring_buffer.pop(update)) {
            ...
            bool basic_anomaly = best_dist > snap->epsilon[best_idx];
            float severity_ratio = (snap->epsilon[best_idx] > 0.0f) ? (best_dist / snap->epsilon[best_idx]) : 0.0f;
            bool severe_anomaly = basic_anomaly && (severity_ratio > VORTEX_SEVERITY_MULTIPLIER);
            Decision decision = decide(snap->regime[best_idx], severe_anomaly);
            ...
            if (basic_anomaly && !in_cooldown && !refit_busy) {
            ...
            } else if (basic_anomaly && in_cooldown) {
...
int main() {
    ...
    printf("DEBUG: peg_repost_count (cancellations due to price drift): %lu\n", peg_repost_count.load());
    printf("\n--- OMS / PnL ---\n");
