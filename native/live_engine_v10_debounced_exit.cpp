...
constexpr uint64_t MIN_ORDER_LIFETIME_TICKS = 100;
constexpr uint64_t MIN_POSITION_HOLD_TICKS = 100;
std::atomic<uint64_t> position_opened_tick{0};
std::atomic<double> latest_best_bid{0.0};
std::atomic<double> latest_best_ask{0.0};
std::atomic<double> latest_bid_volume{0.0};
std::atomic<double> latest_ask_volume{0.0};
...
void close_position_if_open(const BookUpdate& update, uint64_t tick) {
    Position pos = matcher.position();
    if (std::abs(pos.net_quantity) < 0.0005) return;

    uint64_t opened = position_opened_tick.load(std::memory_order_relaxed);
    bool position_is_young = (opened != 0) && (tick - opened < MIN_POSITION_HOLD_TICKS);
    if (position_is_young) return;

    Side closing_side = (pos.net_quantity > 0) ? Side::SELL : Side::BUY;
    double qty = std::abs(pos.net_quantity);

    double ask_prices[1] = {(double)update.P[10]};
    double ask_volumes[1] = {std::max((double)update.dV[10], 0.001)};
    double bid_prices[1] = {(double)update.P[0]};
    double bid_volumes[1] = {std::max((double)update.dV[0], 0.001)};

    printf("[exit] closing position of %.4f via %s market order (regime exited IMPULSE)\n",
           pos.net_quantity, closing_side == Side::SELL ? "SELL" : "BUY");

    matcher.place_market_order(closing_side, ask_prices, ask_volumes, bid_prices, bid_volumes, 1, qty, tick);
    position_opened_tick.store(0, std::memory_order_relaxed);
}
...
void execute_decision(Decision decision, float directed_pressure_proxy, const BookUpdate& update) {
    ...
    switch (decision) {
        case Decision::CANCEL_ALL: {
            uint64_t placed = matcher.oldest_open_order_placed_tick();
            bool order_is_young = (placed != 0) && (tick - placed < MIN_ORDER_LIFETIME_TICKS);
            if (!order_is_young) {
        ...
        case Decision::MARKET_TAKER: {
            Side side = (directed_pressure_proxy > 0) ? Side::BUY : Side::SELL;
            Position current_pos = matcher.position();
            bool already_positioned_this_way =
                (side == Side::BUY && current_pos.net_quantity > 0.0005) ||
                (side == Side::SELL && current_pos.net_quantity < -0.0005);

            if (!already_positioned_this_way) {
                double ask_prices[1] = {best_ask};
                double ask_volumes[1] = {std::max(ask_vol, 0.001)};
                double bid_prices[1] = {best_bid};
                double bid_volumes[1] = {std::max(bid_vol, 0.001)};
                matcher.place_market_order(side, ask_prices, ask_volumes, bid_prices, bid_volumes, 1, 0.001, tick);
                if (position_opened_tick.load(std::memory_order_relaxed) == 0) {
                    position_opened_tick.store(tick, std::memory_order_relaxed);
                }
            }
            break;
        }
        case Decision::LIQUIDITY_PROVISION: {
            close_position_if_open(update, tick);
            ...
            if (matcher.n_open_orders() == 0) {
                ...
                matcher.place_limit_order(Side::BUY, bid_price, 0.001, std::max((double)min_bid_vol, 0.001), tick);
                matcher.place_limit_order(Side::SELL, ask_price, 0.001, std::max((double)min_ask_vol, 0.001), tick);
            }
            break;
        }
        case Decision::HOLD: {
...
void depth_thread_fn() {
    ...
    try {
        ...
        while (!shutdown_flag.load(std::memory_order_relaxed)) {
            ...
            latest_best_bid.store(top_bids[0].price, std::memory_order_relaxed);
            latest_best_ask.store(top_asks[0].price, std::memory_order_relaxed);
            latest_bid_volume.store(top_bids[0].quantity, std::memory_order_relaxed);
            latest_ask_volume.store(top_asks[0].quantity, std::memory_order_relaxed);
...
int main() {
    printf("=== TOPOS Full Reactor: WS Depth + WS Trades + Hot Path + Shadow OMS ===\n");
    printf("WARNING: order book has no REST snapshot bootstrap; Shadow Matcher is a simplified queue model\n\n");
