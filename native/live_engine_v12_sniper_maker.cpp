...
void execute_decision(Decision decision, float directed_pressure_proxy, const BookUpdate& update) {
    ...
    switch (decision) {
        ...
        case Decision::MARKET_TAKER: {
            ...
            // the spread. Instead we cancel any resting order on the wrong
            // side and post a price-improved limit order (one tick better
            // than current best) on the side we expect to be filled from,
            // which makes us the new best price -> queue_position = 0,
            // guaranteeing first-in-line if any trade occurs at or through
            // this price. This directly avoids the queue-depth problem found
            // earlier (25 BTC ahead of us at the old best bid/ask).
            bool expect_up = directed_pressure_proxy > 0;
            Side side = expect_up ? Side::BUY : Side::SELL;
            ...
            matcher.place_limit_order(side, improved_price, ORDER_SIZE, 0.0, tick);
            break;
        }
