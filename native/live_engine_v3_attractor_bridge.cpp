...
void hot_path_thread_fn() {
    ...
    while (!shutdown_flag.load(std::memory_order_relaxed)) {
        if (ring_buffer.pop(update)) {
            ...
            if (anomaly && !refit_in_progress.load(std::memory_order_acquire) && !refit_requested.load(std::memory_order_acquire)) {
                should_trigger_refit = true;
                trigger_reason = "ANOMALY";
            } else if (tick_count_since_last_refit >= DRIFT_REFIT_TICK_INTERVAL
                       && !refit_in_progress.load(std::memory_order_acquire)
                       && !refit_requested.load(std::memory_order_acquire)) {
                should_trigger_refit = true;
