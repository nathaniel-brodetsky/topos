...
std::atomic<uint64_t> vortex_cancel_suppressed_count{0};
...
constexpr bool EXPERIMENT_LET_MAKER_ORDERS_RIDE_THROUGH_VORTEX = true;
...
void execute_decision(Decision decision, float directed_pressure_proxy, const BookUpdate& update) {
    ...
    switch (decision) {
        case Decision::CANCEL_ALL: {
            if (!EXPERIMENT_LET_MAKER_ORDERS_RIDE_THROUGH_VORTEX) {
                matcher.cancel_all(tick);
            } else {
                vortex_cancel_suppressed_count.fetch_add(1, std::memory_order_relaxed);
            }
            break;
        }
        ...
        case Decision::LIQUIDITY_PROVISION: {
            if (matcher.n_open_orders() == 0) {
                matcher.place_limit_order(Side::BUY, best_bid, 0.001, std::max(bid_vol, 0.001), tick);
                matcher.place_limit_order(Side::SELL, best_ask, 0.001, std::max(ask_vol, 0.001), tick);
            }
            break;
        }
        case Decision::HOLD:
...
int main() {
    ...
    if (matcher.n_open_orders() > 0) {
        printf("\n[shutdown] force-canceling %zu open orders before final report (no unrealized exposure left unreported)\n",
    ...
    printf("EXPERIMENT: vortex cancel_all calls suppressed (maker orders left resting): %lu\n", vortex_cancel_suppressed_count.load());
    printf("EXPERIMENT flag EXPERIMENT_LET_MAKER_ORDERS_RIDE_THROUGH_VORTEX = %s\n", EXPERIMENT_LET_MAKER_ORDERS_RIDE_THROUGH_VORTEX ? "true" : "false");
    printf("\n--- OMS / PnL ---\n");
