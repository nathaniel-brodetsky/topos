...
int main() {
    ...
    int pin_result = pthread_setaffinity_np(current_thread, sizeof(cpu_set_t), &cpuset);
    if (pin_result != 0) {
        printf("WARNING: failed to pin to CPU core 2, errno=%d\n", pin_result);
    } else {
        printf("Pinned to CPU core 2\n");
    }
