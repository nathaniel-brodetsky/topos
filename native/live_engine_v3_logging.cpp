...
void hot_path_thread_fn() {
    ...
    logfile.open("/tmp/topos_live_log.csv");
    logfile << "tick,mid_price,commutator_norm,dist,epsilon,anomaly,regime,decision,directed_pressure_proxy,value_pred,snap_version\n";
    ...
    while (!shutdown_flag.load(std::memory_order_relaxed)) {
        if (ring_buffer.pop(update)) {
            ...
            const char* regime_names[] = {"EQUILIBRIUM", "IMPULSE", "TRANSITIONAL"};
            const char* regime_str = anomaly ? "TOXIC_VORTEX" : regime_names[(int)snap->regime[best_idx]];

            logfile << count << ","
                    << (update.P[0] + update.P[10]) / 2.0f << ","
                    << commutator_norm << ","
                    << best_dist << ","
                    << snap->epsilon[best_idx] << ","
                    << (anomaly ? 1 : 0) << ","
                    << regime_str << ","
